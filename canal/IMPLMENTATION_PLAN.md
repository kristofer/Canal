...
**✅ COMPLETED: Phase 3 - Mandatory Service:FS + Phase 5 - Error Hardening + Symbol Definition**
   - `xQueueGenericSend`

- WiFi picoceci REPL now has mandatory `service:fs` operations (no fallback):
  - `Canal capability: #fsRead`, `#fsWrite`, `#fsReadWrite` (defined constants in capability_symbols.go)
  - All FS operations now require real service:fs connection
  - Local in-memory FS fallback completely removed
- Comprehensive error reporting for all failure points:
  - Capability request failures with detailed error messages (access denied, not found, etc.)
  - Capability send/recv failures with timeout reporting
  - FS operation failures (open, read, write, list, sync) with human-readable error context
- All errors propagate to picoceci REPL as readable strings
- Module resolver (`readModuleFromFS`) now mandatory requires real SD card filesystem
  - `xQueueReceive`
  - `vQueueDelete`
...
Original: Replace the local in-memory shim with real `service:fs` operations. **✅ ACHIEVED**
...
**Phase 3: Replace Local FS Shim with Real `service:fs` Calls - COMPLETE**
- Removed localFiles array and all local FS functions
- FS methods now mandatory require service:fs with no fallback
- readModuleFromFS() calls fsReadFile() directly

**Phase 5: Cleanup and Hardening - COMPLETE**
- Removed local in-memory FS fallback entirely
- Added errorCodeToMessage() mapping for all kernel error codes
- Updated all capability syscalls (capRequest, capSend, capDoSyscall) with detailed error reporting
- Updated all FS operations (fsList, fsReadFile, fsWriteFile) with operation-specific error messages
- Added capability_symbols.go defining CapabilityFSRead, CapabilityFSWrite, CapabilityFSReadWrite
- Updated makeCanalObject() to validate symbols and report unknown capabilities
- All errors now propagate to picoceci REPL as human-readable strings

...
// Validate capability
func CapValidate(capID CapabilityID, requiredRights uint32) uint8 {
	if capID >= CapabilityID(maxCapabilities) {
...
func CapGrant(capID CapabilityID, granter, grantee DomainID) uint8 {
	spinLock(&capTableLock)
	defer spinUnlock(&capTableLock)
...
// Revoke capability
func CapRevoke(capID CapabilityID, revoker DomainID) uint8 {
	spinLock(&capTableLock)
	defer spinUnlock(&capTableLock)
...
func CapSend(capID CapabilityID, sender DomainID, data unsafe.Pointer, size uint32) uint8 {
...
func CapRecv(capID CapabilityID, receiver DomainID, data unsafe.Pointer, size uint32) uint8 {

...
type CapHandle = capHandle

const (
	RightRead      = kernel.RightRead
	RightWrite     = kernel.RightWrite
	RightExecute   = kernel.RightExecute
	RightGrant     = kernel.RightGrant
	RightRevoke    = kernel.RightRevoke
	RightReadWrite = kernel.RightRead | kernel.RightWrite
)
...
// CapSendRaw sends a fixed-size message through a capability without marshaling.
func CapSendRaw(handle CapHandle, data unsafe.Pointer, size uintptr) error {
